    ...
    public class EmitterFactory
    {
        ...
        public void SetBehaviourFromXml(XElement data)
        {
            try
            {
                ...
                {
                    spread = MathHelper.ToRadians(Utility.ToFloat(data.Element("burstmetrics").Attribute("spread").Value));
                }
                ...
                SetSpeedMetrics(
                    new Range(
                        Utility.ToFloat(data.Element("speed").Element("launch").Attribute("minimum").Value),
                        Utility.ToFloat(data.Element("speed").Element("launch").Attribute("maximum").Value)),
                    new Range(
                        Utility.ToFloat(data.Element("speed").Element("delta").Attribute("minimum").Value),
                        Utility.ToFloat(data.Element("speed").Element("delta").Attribute("maximum").Value)),
                    new Range(
                        Utility.ToFloat(data.Element("speed").Element("accelerator").Attribute("minimum").Value),
                        Utility.ToFloat(data.Element("speed").Element("accelerator").Attribute("maximum").Value)));

                if (data.Elements("rotation").Count() > 0)
                {
                    SetSpinMetrics(
                        new Range(
                            Utility.ToFloat(data.Element("rotation").Element("launch").Attribute("minimum").Value),
                            Utility.ToFloat(data.Element("rotation").Element("launch").Attribute("maximum").Value)),
                        new Range(
                            Utility.ToFloat(data.Element("rotation").Element("delta").Attribute("minimum").Value),
                            Utility.ToFloat(data.Element("rotation").Element("delta").Attribute("maximum").Value)),
                        new Range(
                            Utility.ToFloat(data.Element("rotation").Element("accelerator").Attribute("minimum").Value),
                            Utility.ToFloat(data.Element("rotation").Element("accelerator").Attribute("maximum").Value)));
                }
                ...
                if (data.Elements("scale").Count() > 0)
                {
                    SetScaleMetrics(
                        new Range(
                            Utility.ToFloat(data.Element("scale").Element("launch").Attribute("minimum").Value),
                            Utility.ToFloat(data.Element("scale").Element("launch").Attribute("maximum").Value)),
                        new Range(
                            Utility.ToFloat(data.Element("scale").Element("delta").Attribute("minimum").Value),
                            Utility.ToFloat(data.Element("scale").Element("delta").Attribute("maximum").Value)),
                        new Range(
                            Utility.ToFloat(data.Element("scale").Element("accelerator").Attribute("minimum").Value),
                            Utility.ToFloat(data.Element("scale").Element("accelerator").Attribute("maximum").Value)));
                }
                ...
                if (data.Elements("fadepoints").Count() > 0)
                {
                    ...
                    {
                        AddFadePoint(Utility.ToFloat(el.Attribute("lifefraction").Value), Utility.ToFloat(el.Attribute("alphafraction").Value));
                    }
                }
                ...
                if (data.Elements("emissionoffsets").Count() > 0)
                {
                    ...
                    {
                        AddEmissionPointOffset(new Vector2(Utility.ToFloat(el.Attribute("x").Value), Utility.ToFloat(el.Attribute("y").Value)));
                    }
                }
            }
            catch
